package com.adrian.games.gobang;
...
public class GobangView extends View {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {// 初始化和屏幕大小改变时调用该函数
        ...
        gridSize = min / AutoGobangActivity.LINE;
    ...
    private void getRect(int x, int y, Rect rect) {
        rect.set((int) (x * gridSize + getTopX()),
                y * gridSize + getTopY(), (int) (x * gridSize
                        + gridSize + getTopX()),
                y * gridSize + gridSize + getTopY());
    }
    ...
    public void select(int x, int y) {// 选择区域
        ...
        selectX = Math.min(Math.max(x, 0), AutoGobangActivity.LINE - 1);
        selectY = Math.min(Math.max(y, 0), AutoGobangActivity.LINE - 1);
        this.getRect(selectX, selectY, selectRect);
        this.invalidate(selectRect);
    }
    ...
    private void drawPieces(Canvas canvas) {
        ...
        for (int i = 0; i < AutoGobangActivity.LINE; i++) {
            for (int j = 0; j < AutoGobangActivity.LINE; j++) {
    ...
    private void drawGridboard(Canvas canvas) {
        ...
        for (int i = 0; i <= AutoGobangActivity.LINE; i++) {
            canvas.drawLine(getTopX(), i * gridSize + getTopY(), getTopX()
                    + gridSize * AutoGobangActivity.LINE, getTopY() + i
                    * gridSize, hilite);
            canvas.drawLine(getTopX() + i * gridSize, getTopY(), i * gridSize,
                    getTopY() + gridSize * AutoGobangActivity.LINE, hilite);
