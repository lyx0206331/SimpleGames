...
/**
 * Created by 卿 on 2016/1/19 0019.
 */
public class GomokuView extends View {
    ...
    public GomokuView(Context context, AttributeSet attrs) { //好像调用的是这个构造函数，为什么不是前面的呢
        super(context, attrs);
    ...
    public void init() {
        ...
        Drawable tile = r.getDrawable(R.drawable.gobang_chess_black);
        tile.setBounds(0, 0, CHESS_DIAMETER, CHESS_DIAMETER);
        tile.draw(canvas);
        mChessBW[0] = bitmap;

        tile = r.getDrawable(R.drawable.gobang_chess_white);
        tile.setBounds(0, 0, CHESS_DIAMETER, CHESS_DIAMETER);
        tile.draw(canvas);
        mChessBW[1] = bitmap;
    }
    ...
    public boolean checkFull() {
        ...
        return mNotEmpty == (GRID_SIZE - 1) * (GRID_SIZE - 1);
    }
